...
import classes from "./SneakersCard.module.scss";

const SneakersCard = (props) => {
  const { image, info, price, sId } = props.sneakers;
  ...
  useEffect(() => {
    const findId = ctx.cart.findIndex((item) => item.sId === sId);
    setIsInCart(findId !== -1);
  }, [ctx.cart, sId]);
  ...
  const setFavouriteHandler = () => {
    if (!isFavorite) sneakersCtx.addFavourite(props.sneakers);
    else sneakersCtx.deleteFavourite(sId);
  };

  return (
    <div className={classes.card}>
      <img src={image} alt={info} width={133} height={112} />
      <button
        type="button"
        className={`${classes.addFavBtn} ${isFavorite ? classes.active : ""}`}
      ...
      <p className={classes.card__info}>{info}</p>
      <div className={classes.card__footer}>
        ...
        <button
          type="button"
          className={`${classes.plusBtn} ${isInCart ? classes.active : ""}`}
          onClick={() => {
            ctx.addItem(props.sneakers);
          }}
        >
          {!isInCart && (
